...
from morphosolver.addons.gf import AnisoGf
from morphosolver.core import SmartFunction, Chemical, GenericDomain

from fenicstools import interpolate_nonmatching_mesh

# The function defying the state of the tissue, and the extent to which it is a source of inhibitor.
class StateFunction(Expression):
	...
	def eval(self, value, x):
		...
		if len(self.notches) > 0:
			...
			centroid = sum(self.notches, Point(0,0)) / len(self.notches)
			dist = 2*centroid.distance(closest)
			
			value[0] = exp(-self.k * (minR/dist)**2 )
	...
	def update(self, tissue, **params):
		...
		for notch in tissue.notches0:
			self.notches.append( notch + Point( tissue.ut( notch ) ) )
